...
class AccountMove(models.Model):
    ...
    payable_move_line_ids = fields.Many2many(
        "account.move.line", string="Payable Move Lines", compute="_compute_payables"
    )

    payment_journal_id = fields.Many2one("account.journal", string="Forma de pagamento")
        ...
        for moveline in self.receivable_move_line_ids:
            ...
            if "errors" in data:
                ...
                msg = "\n".join(
                    ...
                    + ["Field: %s %s" % (x[0], x[1][0]) for x in data["errors"].items()]
                )
                raise UserError(msg)
...
class AccountMoveLine(models.Model):
    ...
    iugu_secure_payment_url = fields.Char(
        string="URL de Pagamento", size=500, copy=False
    )
    ...
    def action_notify_due_payment(self):
        if self.invoice_id:
            self.invoice_id.message_post(body="Notificação do IUGU: Fatura atrasada")
