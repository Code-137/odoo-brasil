...
class AccountInvoice(models.Model):
    ...
    invoice_eletronic_ids = fields.One2many(
        "invoice.eletronic", "invoice_id", u"Documentos Eletrônicos", readonly=True
    )
    invoice_model = fields.Char(
        string="Modelo de Fatura",
        related="product_document_id.code",
        readonly=True,
        store=True,
    )
    ...
    def _action_preview_danfe(self, doc):
        ...
        if not report:
            raise UserError(
                _(
                    "Nenhum relatório implementado para este modelo \
                  de documento"
                )
            )
    ...
    def _prepare_edoc_item_vals(self, line):
        vals = {
            ...
            "pis_valor_retencao": abs(line.pis_valor) if line.pis_valor < 0 else 0,
            # - COFINS -
            ...
            "cofins_valor_retencao": abs(line.cofins_valor)
            ...
            "issqn_valor_retencao": abs(line.issqn_valor)
            if line.issqn_valor < 0
            else 0,
            # - RETENÇÔES -
            "csll_base_calculo": line.csll_base_calculo,
            "csll_aliquota": abs(line.csll_aliquota),
            "csll_valor_retencao": abs(line.csll_valor) if line.csll_valor < 0 else 0,
            "irrf_base_calculo": line.irrf_base_calculo,
            "irrf_aliquota": abs(line.irrf_aliquota),
            "irrf_valor_retencao": abs(line.irrf_valor) if line.irrf_valor < 0 else 0,
            "inss_base_calculo": line.inss_base_calculo,
            "inss_aliquota": abs(line.inss_aliquota),
            "inss_valor_retencao": abs(line.inss_valor) if line.inss_valor < 0 else 0,
        }
        return vals
    ...
    def invoice_validate(self):
        ...
        for item in self:
            ...
            if (
                item.service_document_id.nfse_eletronic
                and not item.company_id.l10n_br_nfse_conjugada
            ):
                inv_lines = item.invoice_line_ids.filtered(
    ...
    def action_cancel(self):
        ...
        for item in self:
            edocs = self.env["invoice.eletronic"].search([("invoice_id", "=", item.id)])
            for edoc in edocs:
                if edoc.state == "done":
                    raise UserError(
                        _(
                            "Documento eletrônico emitido - Cancele o \
...
class AccountInvoiceLine(models.Model):
    ...
    state = fields.Selection(
        string="Status",
        selection=[("pendente", "Pendente"), ("transmitido", "Transmitido"),],
        default="pendente",
    ...
    item_pedido_compra = fields.Char(
        string="Item de compra", size=20, help=u"Item do pedido de compra do cliente"
    )
