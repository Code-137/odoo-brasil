...
class EletronicDocument(models.Model):
    ...
    irrf_base_calculo = fields.Monetary(
        string="Base IRRF", digits="Account", readonly=True, states=STATE
    )
    irrf_valor_retencao = fields.Monetary(
        string="Valor IRRF", digits="Account", readonly=True, states=STATE
    )
    inss_base_calculo = fields.Monetary(
        string="Base INSS", digits="Account", readonly=True, states=STATE
    )
    inss_valor_retencao = fields.Monetary(
        string="Valor INSS", digits="Account", readonly=True, states=STATE
    )
    ...
    schedule_user_id = fields.Many2one(
        "res.users", string="Agendado por", readonly=True, track_visibility="always"
    )
    ...
    fiscal_position_id = fields.Many2one(
        "account.fiscal.position", string=u"Posição Fiscal", readonly=True, states=STATE
    )
    ...
    valor_frete = fields.Monetary(string=u"Total Frete", readonly=True, states=STATE)
    valor_seguro = fields.Monetary(string=u"Total Seguro", readonly=True, states=STATE)
    ...
    valor_bc_icms = fields.Monetary(string=u"Base ICMS", readonly=True, states=STATE)
    valor_icms = fields.Monetary(string=u"Total do ICMS", readonly=True, states=STATE)
    valor_icms_deson = fields.Monetary(
        string=u"ICMS Desoneração", readonly=True, states=STATE
    )
    valor_bc_icmsst = fields.Monetary(
        string=u"Total Base ST",
        help=u"Total da base de cálculo do ICMS ST",
        readonly=True,
        states=STATE,
    )
    valor_icmsst = fields.Monetary(string=u"Total ST", readonly=True, states=STATE)
    valor_ii = fields.Monetary(string=u"Total II", readonly=True, states=STATE)
    valor_ipi = fields.Monetary(string=u"Total IPI", readonly=True, states=STATE)
    ...
    tipo_emissao = fields.Selection(
        [
            ...
            (
                "2",
                u"2 - Contingência FS-IA, com impressão do DANFE em formulário \
         de segurança",
            ),
            ...
            (
                "5",
                u"5 - Contingência FS-DA, com impressão do DANFE em \
    ...
    uf_veiculo = fields.Char(string=u"UF da Placa", size=2, readonly=True, states=STATE)
    rntc = fields.Char(
        string="RNTC",
        size=20,
        readonly=True,
    ...
    fatura_desconto = fields.Monetary(string=u"Desconto", readonly=True, states=STATE)
    ...
    def _compute_legal_information(self):
        ...
        fiscal = self._compute_msg(fiscal_ids) + (self.invoice_id.fiscal_comment or "")
        observacao = self._compute_msg(obs_ids) + (self.invoice_id.comment or "")
    ...
    def log_exception(self, exc):
        self.write(
            {"codigo_retorno": -1, "mensagem_retorno": str(exc), "state": "error",}
        )
    ...
    def _find_attachment_ids_email(self):
        ...
        danfe_report = self.env["ir.actions.report"].search(
            [
                (
                    "report_name",
                    "=",
                    "l10n_br_eletronic_document.main_template_br_nfse_danfpse",
                )
            ]
        )
        ...
        report_name = safe_eval(
            danfe_report.print_report_name, {"object": self, "time": time}
        )
    ...
    def _update_document_values(self):
        self.write(
            {"data_emissao": datetime.now(), "schedule_user_id": self.env.user.id,}
        )
...
class EletronicDocumentLine(models.Model):
    ...
    preco_unitario = fields.Monetary(
        string="Preço Unitário", digits="Product Price", readonly=True, states=STATE
    )
    ...
    frete = fields.Monetary(
        string="Frete", digits="Account", readonly=True, states=STATE
    )
    seguro = fields.Monetary(
        string="Seguro", digits="Account", readonly=True, states=STATE
    )
    desconto = fields.Monetary(
        string="Desconto", digits="Account", readonly=True, states=STATE
    )
    outras_despesas = fields.Monetary(
        string="Outras despesas", digits="Account", readonly=True, states=STATE
    )

    tributos_estimados = fields.Monetary(
        string="Valor Estimado Tributos", digits="Account", readonly=True, states=STATE
    )

    valor_bruto = fields.Monetary(
        string="Valor Bruto", digits="Account", readonly=True, states=STATE
    )
    valor_liquido = fields.Monetary(
        string="Valor Líquido", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_base_calculo = fields.Monetary(
        string="Base de cálculo", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_valor = fields.Monetary(
        string="Valor Total", digits="Account", readonly=True, states=STATE
    )
    icms_valor_credito = fields.Monetary(
        string="Valor de Cŕedito", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_valor_diferido = fields.Monetary(
        string="Valor Diferido", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_valor_desonerado = fields.Monetary(
        string="Valor Desonerado", digits="Account", readonly=True, states=STATE
    )
    ...
    ii_valor_despesas = fields.Monetary(
        string="Despesas Aduaneiras", digits="Account", readonly=True, states=STATE
    )
    ii_valor = fields.Monetary(
        string="Imposto de Importação", digits="Account", readonly=True, states=STATE
    )
    ii_valor_iof = fields.Monetary(
        string="IOF", digits="Account", readonly=True, states=STATE
    )

    # ------------ PIS ---------------------
    pis_cst = fields.Selection(
        CST_PIS_COFINS, string="Situação Tributária", readonly=True, states=STATE
    )
    ...
    pis_valor_retencao = fields.Monetary(
        string="Valor Retido", digits="Account", readonly=True, states=STATE
    )
    ...
    iss_valor_retencao = fields.Monetary(
        string="Valor Retenção", digits="Account", readonly=True, states=STATE
    )

    # ------------ RETENÇÔES ------------
    csll_base_calculo = fields.Monetary(
        string="Base CSLL", digits="Account", readonly=True, states=STATE
    )
    csll_aliquota = fields.Float(
        string="Alíquota CSLL", digits="Account", readonly=True, states=STATE
    )
    csll_valor = fields.Monetary(
        string="Valor CSLL", digits="Account", readonly=True, states=STATE
    )
    csll_valor_retencao = fields.Monetary(
        string="Retenção CSLL", digits="Account", readonly=True, states=STATE
    )
    irpj_base_calculo = fields.Monetary(
        string="Base IRPJ", digits="Account", readonly=True, states=STATE
    )
    irpj_aliquota = fields.Float(
        string="Alíquota IRPJ", digits="Account", readonly=True, states=STATE
    )
    irpj_valor = fields.Monetary(
        string="Valor IRPJ", digits="Account", readonly=True, states=STATE
    )
    irpj_valor_retencao = fields.Monetary(
        string="Retenção IRPJ", digits="Account", readonly=True, states=STATE
    )
    irrf_base_calculo = fields.Monetary(
        string="Base de Cálculo", digits="Account", readonly=True, states=STATE
    )
    ...
    irrf_valor_retencao = fields.Monetary(
        string="Retenção IRRF", digits="Account", readonly=True, states=STATE
    )
    inss_base_calculo = fields.Monetary(
        string="Base INSS", digits="Account", readonly=True, states=STATE
    )
    ...
    inss_valor_retencao = fields.Monetary(
        string="Retenção INSS", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_bc_uf_dest = fields.Monetary(string=u"Base ICMS", readonly=True, states=STATE)
    ...
    icms_uf_dest = fields.Monetary(string=u"ICMS Destino", readonly=True, states=STATE)
    icms_fcp_uf_dest = fields.Monetary(string=u"Valor FCP", readonly=True, states=STATE)
