...
class EletronicDocument(models.Model):
 ...
 SEM VALOR FISCAL"
        ...
        total = {
            ...
            "vNF": "%.02f" % sum(self.document_line_ids.mapped("valor_liquido")),
        ...
        if responsavel_tecnico:
            if len(responsavel_tecnico.child_ids) == 0:
                raise UserError("Adicione um contato para o responsável técnico!")
        ...
        if self.valor_servicos > 0.0:
            vals.update(
                {"ISSQNtot": issqn_total, "retTrib": tributos_retidos,}
            )
    ...
    def action_send_eletronic_invoice(self):
        ...
        if retorno.cStat != 104:
            self.write(
                {"codigo_retorno": retorno.cStat, "mensagem_retorno": retorno.xMotivo,}
            )
            self.notify_user()
        else:
            self.write(
