...
class EletronicDocument(models.Model):
    ...
    def get_ide(self, nfe, operacao):
        ...
        return dict(
            ...
            data_entrada_saida=data_entrada_saida.astimezone(pytz.utc).replace(
                tzinfo=None
            ),
    ...
    def create_invoice_eletronic_item(
        ...
        seller_id = self.env["product.supplierinfo"].search(
            [("name", "=", partner_id), ("product_code", "=", codigo)]
        )
    ...
    def _prepare_account_invoice_vals(
        self,
        company_id,
        tax_automation=False,
        supplierinfo_automation=False,
        ...
        journal_id = (
            ...
            .with_context(type=operation, company_id=company_id.id)
        ...
        if not journal_id:
            raise UserError(
                _(
                    "Please define an accounting sale journal for\
                    this company."
                )
            )
        vals = {
            ...
            "date_invoice": datetime.strftime(self.data_emissao, "%Y-%m-%d %H:%M:%S"),
    ...
    def prepare_account_invoice_vals(
        ...
        account_invoice.message_post(
            body=u"<ul><li>Fatura criada através da do xml\
                                     da NF-e %s</li></ul>"
            % self.numero
        )
    ...
    def prepare_account_invoice_line_vals(
        ...
        if purchase_line_id:
            ...
            tax_ipi_id = taxes_id.filtered(lambda x: x.domain == "ipi" and x.amount > 0)
            tax_pis_id = taxes_id.filtered(lambda x: x.domain == "pis" and x.amount > 0)
            ...
            vals = {
                ...
                "icms_aliquota_reducao_base": purchase_line_id.icms_aliquota_reducao_base,
                "icms_aliquota_credito": purchase_line_id.aliquota_icms_proprio,
                "icms_st_aliquota_mva": purchase_line_id.icms_st_aliquota_mva,
                "icms_st_aliquota_reducao_base": purchase_line_id.icms_st_aliquota_reducao_base,
                "icms_st_aliquota_deducao": purchase_line_id.icms_st_aliquota_deducao,
            ...
            vals = {
                ...
                "cfop_id": cfop.id,
                "icms_csosn_simples": item.icms_cst
                if item.icms_cst and len(item.icms_cst) == 3
                else "",
                "icms_cst": item.icms_cst
                if item.icms_cst and len(item.icms_cst) == 2
                else "",
                ...
                "icms_st_aliquota_reducao_base": item.icms_st_aliquota_reducao_base,
                "icms_st_aliquota_deducao": item.icms_st_aliquota_reducao_base,
                "issqn_aliquota": item.issqn_aliquota if item.issqn_aliquota else "",
                "issqn_base_calculo": item.issqn_base_calculo
                if item.issqn_base_calculo
                else "",
        ...
        vals.update(
            {
                ...
                "tax_icms_st_id": None if tax_icms_st_id is None else tax_icms_st_id.id,
                ...
                "tax_cofins_id": None if tax_cofins_id is None else tax_cofins_id.id,
                "tax_ii_id": None,
                "tax_issqn_id": None if tax_issqn_id is None else tax_issqn_id.id,
    ...
    def _create_tax(self, tax_domain, aliquota, company_id):
        ...
        message = (
            u"<ul><li>Aliquota criada através da importação\
    ...
    def _create_supplierinfo(self, item, purchase_order_line, automation=False):
        ...
        if not supplierinfo_id:
            ...
            message = (
                u"<ul><li>Produto do fornecedor criado através da\
